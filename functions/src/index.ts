...
import { processAttendanceEvent } from "./attendance/processAttendanceEvent";
import { processNewStudent } from "./student/processNewStudent";
// Initialize Firebase Admin only once
if (!getApps().length) {
  initializeApp();
}

// Get database ID from environment variable, default to "(default)" for Firestore Native
...
export const scheduledExecutiveReport = pubsub
  .schedule("29 18 * * *")
  .onRun(async (context) => {
    console.log(
      "Scheduled function 'scheduledExecutiveReport' started at",
    ...
    const totalAmountPaid = todaysPayments.reduce(
    ...
    todaysPayments.forEach((payment) => {
      ...
      amountByReceivedBy[receivedBy] =
        (amountByReceivedBy[receivedBy] || 0) + Number(payment.amountPaid || 0);
    });
    ...
    const totalExpenseAmount = todaysExpenses.reduce(
      (sum, expense) => sum + Number(expense.expenseAmount || 0),
      0,
    );
    ...
    const media_url =
      "https://ik.imagekit.io/acak7duni/Executive%20Summary.jpg?updatedAt=175742657752";
    const message_id = "5358";
    const authorization =
      "5JFElxhK0QfoLzBwuvVIS1CnDqdmHWNr26y98AXeTYRkipbGUgQgXTJ7SKsbGAwOBc0nPDejW3lkrNI8";
...
// On New Admission
export const onNewStudentEvent = firestore
  .document("STUDENTS/{studentId}")
  .onCreate(async (snap, context) => {
    const event = snap.data() as any;
    console.log("Processing new student event:", event);
    try {
      await processNewStudent(event);
      console.log("New Student processed:", context.params.studentId);
    } catch (err) {
      console.error("Error processing attendance:", err);
    }
  });
